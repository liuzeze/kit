...
public class LzDisplayUtils {
    ...
    private static final String TAG = "LzDisplayUtils";
    ...
    public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isLandscape && QMUIDeviceHelper.isEssentialPhone()
                ...
                result -= 2 * LzStatusBarHelper.getStatusbarHeight(context);
            }
            return result;
        }
        if (isLandscape) {
            // 华为挖孔屏横屏时，会把整个 window 往后移动，因此，可用区域减小
            if (QMUIDeviceHelper.isHuawei() && !LzDisplayUtils.huaweiIsNotchSetToShowInSetting(context)) {
